	...
	public class Player : Entity, IMcpeMessageHandler
	{
		...
		public virtual void HandleMcpeInteract(McpeInteract message)
		{
			...
			if (player != null)
			{
				...
				player.HealthManager.TakeHit(this, damage, DamageCause.EntityAttack);
			}
			else
			{
		...
		protected virtual double CalculateDamage(Entity target)
		{
			double damage = Inventory.GetItemInHand().GetDamage(); //Item Damage.
